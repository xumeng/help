    ...
    for (let index2 = 0; index2 < andArr.length; index2++) {
      ...
      if (andItem.indexOf("!=") > -1) {
        ...
        if (isNaN(value)) {
          ...
          if (typeof data[key] !== "undefined")
            itemKey = data[key] <= Number(value) ? true : false;
          if (typeof data[key] === "undefined" || data[key] === null)
            itemKey = false;
        }
      } else if (andItem.indexOf("<") > -1) {
...
pubfn.calcFreightDetail = function (freightItem, weight) {
  ...
  let total_amount =
    first_weight_price * first_weight_count +
  ...
  for (const key in obj) {
    if (Object.prototype.hasOwnProperty.call(obj, key)) {
      if (parserReg.test(key)) {
        ...
        delete obj[key];
        if (
          Object.prototype.toString.call(obj[keyCopy]) === "[object Object]"
        ) {
 ...
 */
pubfn.percentageFilter = function (
  value,
  needShowSymbol = true,
  defaultValue = ""
) {
...
pubfn.getComponentsDynamicData = function (obj = {}) {
  ...
  callFunction({
    ...
    success: function (data) {
      if (
        JSON.stringify(cacheData[cacheDataKey]) !==
        JSON.stringify(data.componentsDynamic)
      ) {
        // 渲染服务器数据
...
pubfn.fileToBase64 = function (obj = {}) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    plus.io.resolveLocalFileSystemURL(
      pubfn.getLocalFilePath(filePath),
      function (entry) {
        entry.file(
          function (file) {
...
pubfn.base64ToFile = function (obj = {}) {
  ...
  if (!filePath) {
    filePath =
      pubfn.random(32, "abcdefghijklmnopqrstuvwxyz0123456789") + "." + extName;
  }
...
pubfn.getLocaleList = function () {
  let localeList = [{ value: "zh-Hans", label: "简体中文" }];
  return localeList;
};
/**
 * 获取当前应用语言
